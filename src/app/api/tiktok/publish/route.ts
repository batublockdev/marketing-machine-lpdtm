...
// Required body fields per TikTok guidelines:
// - postId: string
// - title: string (user editable)
// - privacyLevel: string (user selected from privacy_level_options)
// - allowComment: boolean (user selected, default false)
// - allowDuet: boolean (user selected, default false)
// - allowStitch: boolean (user selected, default false)
// - commercialContent: boolean (default false)
// - brandOrganic: boolean (if commercialContent is true)
// - brandedContent: boolean (if commercialContent is true)
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate required fields per TikTok guidelines
    if (!title || !privacyLevel) {
      return NextResponse.json({
        success: false,
        error: 'Title and privacyLevel are required per TikTok guidelines'
      }, { status: 400 });
    }
    ...
    // Get creator info first (required by TikTok guidelines)
    ...
    // Validate privacy level is in allowed options
    ...
    const maxDuration = creatorData.data?.max_video_post_duration_sec || 300;
    // TODO: Check actual video duration
    ...
    if (postId) {
      await prisma.post.update({
        ...
        data: {
          ...
          publishedUrl: `https://www.tiktok.com/@${token.openId}/video/${publish_id}`
        }
      });
    }
...
// Helper: Get video path from post
